...
<html>
  ...
  <body>
    ...
    const selectedDatee = moment(this.state.selectedDate).format(
      'DD_MM_YYYY_HH_mm_ss',
    );
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.branchHeader}>SANTUKA TRANSPORT</Text>
        <Text style={styles.branchSubHeader}>NIE, JAGATPUR, CUTTACK</Text>
          ...
          <View>
            <Table borderStyle={{borderColor: 'black'}}>
              <Row
                ...
                style={[styles.header, {borderWidth: 1}]}
            ...
            <ScrollView style={styles.dataWrapper}>
              {state.dataAvailable ? (
                state.tableData.map((rowData, index) => {
                  const slNo = index + 1;
                  return (
                    <Table borderStyle={{borderColor: 'black'}} key={index}>
                      <Row
                        data={state.tableHead.map(header => {
                          if (header === 'LoadDate') {
                            ...
                            const columnName =
                              state.columnDataMapping[header] || header;
                            const cellValue =
                              header === 'SL NO'
                                ? slNo
                                : rowData[columnName] !== undefined
                                ? ['Cash', 'Hsd', 'E-Adv'].includes(header) &&
                                  parseFloat(rowData[columnName]) === 0
                ...
                <Text style={styles.noDataText}>
                  Data not available for the selected date.
                </Text>
              )}
                  ...
                  <Row
                    data={state.tableHead.map(header => {
                      if (header === 'SL NO') {
                        ...
                        return loadingQtySum > 0
                          ? loadingQtySum.toFixed(3)
                          : ''; // Display empty cell if sum is not available
                      } else {
                        return '';
                      }
                    })}
...
const styles = StyleSheet.create({
  ...
  branchSubHeader: {fontSize: 18, textAlign: 'center', color: 'black'},
  inputContainer: {
    flexDirection: 'row',
